 ...
 */
function sendRequest(url, method, data, successFunction) {
    $.ajax({
        url: url,
        method: method,
...
(function($) {
    ...
    $(document).on('click', '.product-card-add-to-cart', function (e) {
        e.preventDefault()
        addProductToOrder($(this).data('product-id') , 1)
    });

    $('.logout-btn').click(function (e) {
        e.preventDefault();

        sendRequest('/api/logout', 'POST', '', function () {
            location.href = '/'
        })
    });
    ...
    $('.slider-active-2').owlCarousel({
        ...
        animateOut: 'fadeOut',
        animateIn: 'fadeIn',
        navText: ['<img src="img/icon-img/57.png"> next', 'prev <img src="img/icon-img/58.png">'],
        item: 1,
        responsive: {
            0: {
                items: 1
